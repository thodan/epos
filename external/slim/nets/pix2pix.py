...
# =============================================================================
"""Implementation of the Image-to-Image Translation network.
...
class Block(
  ...
  U-Net network. In particular, each block represents a single operation in the
...
def pix2pix_generator(net,
  ...
  Returns:
    A `Tensor` representing the network output and a dictionary of network end
      points.
...
def pix2pix_discriminator(net, num_filters, padding=2, pad_mode='REFLECT',
  ...
  Args:
    ...
    is_training: Whether or not the network is training or testing.

  Returns:
    A logits `Tensor` of size [batch_size, N, N, 1] where N is the number of
    'patches' we're attempting to discriminate and a dictionary of network end
    points.
  """
  del is_training
  end_points = {}
