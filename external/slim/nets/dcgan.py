...
def discriminator(inputs,
  ...
  Construct discriminator model from inputs to the final endpoint.

  Args:
    inputs: A tensor of size [batch_size, height, width, channels]. Must be
      floating point.
    depth: Number of channels in first convolution layer.
    is_training: Whether the model is for training or not.
    reuse: Whether or not the model variables should be reused. `scope`
  ...
  Returns:
    ...
    end_points: a dictionary from components of the model to their activation.
...
def generator(inputs,
  ...
  Construct generator model from inputs to the final endpoint.
  ...
  Args:
    ...
    reuse: Whether or not the model has its variables should be reused. scope
      must be given to be reused.
    scope: Optional variable_scope.
    fused_batch_norm: If `True`, use a faster, fused implementation of
      batch norm.
