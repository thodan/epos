...
def main(unused_argv):
  ...
  visualwakewords_annotations_train = os.path.join(
      FLAGS.infer_dir, 'instances_visualwakewords_train2014.json')
  visualwakewords_annotations_val = os.path.join(
      FLAGS.infer_dir, 'instances_visualwakewords_val2014.json')
  visualwakewords_labels_filename = os.path.join(FLAGS.infer_dir,
                                                 'labels.txt')
  ...
  if not tf.gfile.IsDirectory(FLAGS.infer_dir):
    tf.gfile.MakeDirs(FLAGS.infer_dir)
  ...
  if not tf.gfile.IsDirectory(FLAGS.infer_dir):
    tf.gfile.MakeDirs(FLAGS.infer_dir)
  train_output_path = os.path.join(FLAGS.infer_dir, 'train.record')
  val_output_path = os.path.join(FLAGS.infer_dir, 'val.record')
